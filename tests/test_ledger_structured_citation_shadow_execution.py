...
class GoalAExecutionAuthorizationTests(unittest.TestCase):
    ...
    def test_execution_ledger_file_matches_code_registry(self) -> None:
        path = ROOT / DEFAULT_EXECUTION_LEDGER_PATH
        payload = json.loads(path.read_text(encoding="utf-8"))
        self.assertEqual(payload["schema_version"], EXECUTION_LEDGER_SCHEMA_VERSION)
        self.assertEqual(payload["kind"], "execution_authorization_ledger")
        recorded = payload["records"][GOAL_A_CONFIG_HASH]
        self.assertEqual(recorded, goal_a_execution_record())
        self.assertEqual(SHADOW_EXECUTION_LEDGER[GOAL_A_CONFIG_HASH], goal_a_execution_record())
    ...
    def test_mini_world_configs_remain_executable(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            ...
            self.assertTrue(execution_authorized(config))
            refuse_unauthorized_shadow_execution(
                config,
                output_dir=root / "out",
                preflight_output_dir=root / "preflight",

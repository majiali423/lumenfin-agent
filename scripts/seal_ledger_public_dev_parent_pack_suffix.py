#!/usr/bin/env python3
"""Validate and publish the LEDGER parent-pack suffix recoverability counts."""
...
from lumenfin.eval.holdout.ledger_parent_probe import PACK_STRATEGIES
from lumenfin.eval.holdout.ledger_parent_return import SCHEMA_VERSION
...
def _script_sha256() -> str:
    return hashlib.sha256(
        (ROOT / "scripts" / "run_ledger_public_dev_parent_pack_suffix.py")
...
def _validate(*, aggregate_path: Path, per_case_path: Path) -> bytes:
    aggregate, raw = _load_json_bytes(aggregate_path, label="suffix probe aggregate")
    try:
        per_case_raw = per_case_path.read_bytes()
        rows = paired_cli._parse_jsonl_text(
            per_case_raw.decode("utf-8"),
            label="suffix probe per-case",
        )
    except (OSError, UnicodeDecodeError) as exc:
        raise HoldoutError("cannot read suffix probe per-case") from exc
    expected = {
    ...
    if (
        ...
        or aggregate.get("selection", {}).get("cases") != 200
        or len(rows) != 200
        or aggregate.get("recovered_cases") != expected
        or aggregate.get("recommended_next_workstream")
        != suffix_cli.recommend_next(rows)
        ...
        raise HoldoutError("parent suffix probe artifact failed sealed validation")
    for row in rows:
        if row.get("row_sha256") != suffix_cli._row_sha256(row):
            raise HoldoutError("suffix per-case identity failed sealed validation")
    return raw
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if output.exists():
            raise HoldoutError("refusing to overwrite sealed suffix probe artifact")
        raw = _validate(
        ...
        safe = (
            ...
            else HoldoutError(f"suffix probe seal failed: {type(exc).__name__}")
        )

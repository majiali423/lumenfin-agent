...
def _read_jsonl(path: Path) -> list[dict[str, Any]]:
    ...
    for line_no, line in enumerate(raw.splitlines(), start=1):
        ...
        if not isinstance(payload, dict):
            raise FinanceBenchLoadError(
                f"expected object on line {line_no} of {path.name}"
            )
        rows.append(payload)
    return rows
...
def discover_financebench_paths(source_dir: str | Path) -> DatasetPaths:
    ...
    questions_path = next((path for path in candidates if path.is_file()), None)
    if questions_path is None:
        raise FinanceBenchLoadError(
            "financebench_open_source.jsonl not found under "
            f"{root} or {root / 'data'}"
        )
    ...
    pdf_candidates = [
        root / "pdfs",
        questions_path.parent.parent / "pdfs",
        root / "data" / "pdfs",
    ]
...
def zero_to_one_page(page_zero: int) -> int:
    """Convert FinanceBench zero-indexed pages to LumenFin 1-indexed pages."""
...
def validate_open_source_count(
    ...
    *,
    ...
    if expected and count != expected:
        raise FinanceBenchLoadError(
            f"expected {expected} open-source questions, found {count}"
        )
...
def load_financebench_dataset(
    ...
    *,
    ...
    documents = load_document_information(paths.documents_path)
    questions = load_questions(paths.questions_path, documents=documents)

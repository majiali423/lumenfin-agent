...
def git_snapshot(repo_root: Path) -> dict[str, Any]:
    def _run(args: list[str]) -> str:
        result = subprocess.run(args, cwd=str(repo_root), capture_output=True, text=True, check=False)
...
def compare_mode_dirs(dirs: list[Path]) -> dict[str, Any]:
    per_case: dict[str, list[dict[str, Any]]] = {}
    for directory in dirs:
        metrics_path = directory / "metrics.json"
        cases_path = directory / "per_case.jsonl"
        mode = directory.name
        if metrics_path.is_file():
            payload = json.loads(metrics_path.read_text(encoding="utf-8"))
            env = payload.get("environment") or {}
            mode = str(env.get("mode") or payload.get("mode") or mode)
        per_case[mode] = read_jsonl(cases_path)
    return compare_modes(per_case)
...
def aggregate_case_metrics(cases: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        ...
        "recall_equals_hit_note": (
            "此时 Recall@K 与 Hit@K 等价。"
            if single_gold
            else "Gold page count varies; Recall@K and Hit@K differ."
        ),
        "page": {key: round(value, 4) for key, value in page.items()},
        "chunk": {key: round(value, 4) for key, value in chunk.items()},
        "bootstrap_95": {
...
def breakdowns(cases: list[dict[str, Any]]) -> dict[str, dict[str, Any]]:
    ...
    for case in cases:
        ...
        for key in groups:
            label = str(labels.get(key, False))
            groups[key].setdefault(label, []).append(case)
    return {
        key: {
            label: {
                "cases": len(items),
                "page_hit_at_5": round(mean(float(item["page"]["hit_at"]["5"]) for item in items), 4),
...
def compare_modes(per_case_by_mode: dict[str, list[dict[str, Any]]]) -> dict[str, Any]:
    ...
    for case_id, mode_rows in sorted(by_id.items()):
        ranks = {
            mode: int(mode_rows[mode]["page"]["first_relevant_rank"] or 0) if mode in mode_rows else 0
            for mode in modes
        }
        hits = {
        ...
        if all(value <= 0 for value in ranks.values()):
            ...
            kind = "unchanged"
        movements.append({"case_id": case_id, "kind": kind, "ranks": ranks, "hit_at_5": hits})
...
def render_markdown(report: dict[str, Any]) -> str:
    ...
    lines.extend(["", str(summary.get("recall_equals_hit_note") or ""), "", "## Chunk-level metrics", "", "| Metric | Value |", "|---|---:|"])
    for key, label in (
        ("hit_at_5", "Hit@5"),
        ("hit_at_10", "Hit@10"),
        ("hit_at_20", "Hit@20"),
    ...
    lines.extend(
        [
            ...
            "",
            "Unrun full held-out numbers are marked `NOT_RUN` / `UNVERIFIED`.",

...
from .schema import CaseQrels, ChunkQrel, FinanceBenchQuestion, GoldPage
...
def map_chunks_to_qrels(
    ...
    """Map indexed chunks onto gold pages with a fail-closed provenance rule.

    Page-derived relevance: same document AND same gold page.
    Evidence-span relevance: same document AND auditable overlap with evidence_text.
    A chunk may receive both labels. Missing page metadata is not a silent
    substitute for span overlap.
    """
    notes: list[str] = []
                ...
                if key not in seen:
                    ...
                    qrels.append(
                        ChunkQrel(
                            ...
                            doc_name=span.evidence_doc_name,
                            page_one=page,
                            match_reason="page_cover",
                        )
                    )
            if auditable_span_overlap(str(chunk.get("text") or ""), span.evidence_text):
                span_mapped.add(span_index)
                key = (chunk_id, "span_overlap")
                if key not in seen:
                    seen.add(key)
                    qrels.append(
                        ChunkQrel(
    ...
    page_chunk_ids = tuple(
        dict.fromkeys(item.chunk_id for item in qrels if item.match_reason == "page_cover")
    )
    span_chunk_ids = tuple(
        dict.fromkeys(item.chunk_id for item in qrels if item.match_reason == "span_overlap")
    )
    span_unmapped_count = 0
    for span_index, _span in enumerate(question.evidence):
        if span_index in span_mapped:
            continue
        span_unmapped_count += 1
        notes.append("span_qrel_unmapped")
    ...
    return CaseQrels(
        ...
        span_chunk_ids=span_chunk_ids,
        span_mapped_count=len(span_mapped),
        span_unmapped_count=span_unmapped_count,
    )


def retrieved_page_keys(hits: list[dict[str, Any]]) -> list[tuple[str, int]]:
    """Collapse chunk hits to first-occurrence (doc, 1-indexed page) ranks."""
